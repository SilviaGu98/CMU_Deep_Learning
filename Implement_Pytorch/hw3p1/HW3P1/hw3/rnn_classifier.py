...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        # TODO
        for s in range(seq_len): #iterate time
            x = self.x[:,s,:]
            hiddens = []
            for l in range(self.num_layers): # iterate layer 
                h_prev_l = self.rnn[l].forward(x, self.hiddens[-1][l]) # h_prev_t
                hiddens.append(h_prev_l)
                x = h_prev_l
            self.hiddens.append(hiddens.copy()) # append the hidden states of every layer
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        """
        for s in range(seq_len-1, -1, -1): # iterate time
            for l in range(self.num_layers-1, -1, -1): # iterate layer
                if l == 0:
                    h_prev_l = self.x[:,s,:]
                else:
